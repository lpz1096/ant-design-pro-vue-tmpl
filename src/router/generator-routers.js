...
export const generatorDynamicRouter = (token) => {
  return new Promise((resolve, reject) => {
    ...
    loginService.getCurrentUserNav(token).then(res => {
      ...
      const childrenNav = []
      //      后端数据, 根级树数组,  根级 PID
      // parentId初始化为0，代表一级菜单   先统计一级菜单
      listToTree(result, childrenNav, 0)
      // 将后端数据合成根级树数组 添加到根级菜单rootRouter
      rootRouter.children = childrenNav
      // menuNav后端数据形成的树数组 还差格式化
      menuNav.push(rootRouter)
      console.log('menuNav', menuNav)
      // 按照vue-router格式化数据 其中component根据后端名称找到对应的页面
      const routers = generator(menuNav)
      routers.push(notFoundRouter)
      // routers最终按照后端数据形成的vue-router树数组
...
const listToTree = (list, tree, parentId) => {
  ...
  list.forEach(item => {
    ...
    if (item.parentId === parentId) {
      ...
      listToTree(list, child.children, item.id)// 一级菜单循环完毕 然后循环它的子菜单 添加到children
      // 删掉不存在 children 值的属性
